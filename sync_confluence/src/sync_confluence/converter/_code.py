...
def _plain_text_macro(
    ...
    macro = AC("structured-macro", {qname("ac", "name"): macro_name})
    if language:
        macro.append(AC("parameter", language, {qname("ac", "name"): "language"}))
...
def _build_macro(
    name: Optional[str], text: str, options: ConverterOptions
) -> ElementType:
...
def transform_code_blocks(root: ElementType, options: ConverterOptions) -> None:
    """Replace every ``<pre><code>`` block with a Confluence macro."""
    for pre in list(root.iter("pre")):
        code = pre.find("code")
        if code is None:
            continue
        macro = _build_macro(_language_name(code), "".join(code.itertext()), options)
        macro.tail = pre.tail
