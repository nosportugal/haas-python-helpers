...
# Field names considered sensitive (case-insensitive matching)
...
class PIIRedactionProcessor:
    ...
    def scrub_dict(self, data: dict) -> dict:
        ...
        return {
            k: _REDACTED if not self.is_safe(k) else v
...
class PIIRedactingLogProcessor:
    """Wrap an OTEL LogRecordProcessor to redact attributes before export.

    Not a true OTEL Processor — we use it as a decorator around the batch
    processor or inside custom export hooks where supported.
    """

    def __init__(self, processor: object, redaction: PIIRedactionProcessor) -> None:
